...
class ETMapi():
    ...
    def __init__(self, area_code='nl', scenario_id=None, end_year=2050, source="api-WB", url_base=None):
    ...
    def _change_inputs(self):
        ...
        url = '/scenarios/'
        p = self.session.put(url + self.scenario_id, json = input_data,headers=self.headers)
    ...
    def generate_input_worksheet(self, filepath=None, scenario_list=None, prettify=False):
        ...
        for sid in scenario_list:
            ...
            scenario = self.get_all_inputs(outputformat="dataframe", scenario_id=self.scenario_id)
        ...
        if len(str(scenario_id)) == 4 and scenario_id is not None:          
            ...
            for script in scripts:
                ...
                if string is not None and string.find('preset_scenario_id')>0:
                    start = string.find('preset_scenario_id') +len('preset_scenario_id') + 2
                    end = start + 6
